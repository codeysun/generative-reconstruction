...
class MultiviewDiffusionGuidance(BaseModule):
    ...
    def configure(self) -> None:
        ...
        self.model.control_scales = [self.cfg.control_scale] * 13
    ...
    def dilate_and_feather_mask(self, mask, dilate_size=1):
        B, C, H, W = mask.shape
    ...
    def forward(
        ...
        **kwargs,
        ...
        if "control" in kwargs:
            ...
            if "mask" in kwargs["control"]:
                control_mask: Float[Tensor, "B 1 H W"] = kwargs["control"]["mask"].permute(0, 3, 1, 2)
                control_mask = self.dilate_and_feather_mask(control_mask, dilate_size=1)
                # bbox_mask = kwargs["control"]["bbox_mask"].permute(0, 3, 1, 2)
