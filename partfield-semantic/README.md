...
This repo is adapted from PartField, with added support for VLM-based semantic labeling
...
2. Perform the semantic segmentation pipeline with `semantic_seg.sh`
3. Extract your desired semantic segments with `mask_selection.py`

## Example Run

First, run PartField inferencing and clustering
...
Then, build a hierarchical segment tree.

```
python tree_vis.py --root exp_results/partfield_features/objaverse --dump_dir exp_results/clustering/objaverse --source_dir data/objaverse_samples --use_agglo True --max_num_clusters 20 --option 0
```

The above script automatically render 360 degree views with `nvdiffrast`. However, if you would like higher quality renders with Blender, run

```
python tree_render_views.py --out_dir exp_results/clustering/objaverse --source_dir data/objaverse_samples
```
...
Or, do everything above with the `semantic_seg.sh` script!

Finally, to extract the masks you want, run:

```
python mask_selection.py --out_dir exp_results/clustering/objaverse
```
...
To compute mIoU, precision, recall, and f1 metrics:

```
python compute_semantic.py --pred_sem_path exp_results/clustering/objaverse/sem_preds
```
