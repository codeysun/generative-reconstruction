...
from tree import PartTree, save_tree
...
def hierarchical_clustering_labels(children, n_samples, max_cluster=20):
    ...
    # List of lists of labels for every point
...
def create_colored_submesh(mesh: trimesh.Trimesh, face_indices) -> trimesh.Trimesh:
    new_mesh = mesh.copy()

    # Initialize vertex colors to black (RGBA)
    new_mesh.visual.vertex_colors = np.zeros((len(new_mesh.vertices), 4), dtype=np.uint8)

    new_mesh.visual.vertex_colors[:, 3] = 255

    # Find the vertices associated with the faces to change
    vertex_indices_to_change = np.unique(new_mesh.faces[face_indices].flatten())

    # Set the color of those vertices to white (RGBA)
    new_mesh.visual.vertex_colors[vertex_indices_to_change] = [255, 255, 255, 255]

    return new_mesh

def hierarchical_clustering_to_tree(mesh: trimesh.Trimesh, hierarchical_labels: np.ndarray) -> PartTree:
    """
    Convert the hierarchical clustering into a part tree structure
    """
    if not mesh.visual:
        mesh.visual.vertex_colors = np.ones((len(mesh.vertices), 4), dtype=np.uint8) * 255
    root_label = hierarchical_labels[-1][0]
    root_depth = len(hierarchical_labels)-1
    parttree = PartTree((root_depth, root_label), mesh)

    label_depths = {root_label: root_depth}

    # traverse hierarchical_labels backwards
    # compare labels with parent labels. Whichever new label exists is the new child
    for i in reversed(range(len(hierarchical_labels) - 1)):
        labels = hierarchical_labels[i]
        parent_labels = hierarchical_labels[i+1]

        changed_indices = np.where(labels != parent_labels)[0] # np.where returns a tuple

        # for each changed index, identify the parent label
        parent_label = parent_labels[changed_indices]
        new_label = labels[changed_indices]

        # Assert that only one cluster was created, from the same parent cluster
        assert np.unique(new_label).shape[0] == 1, "More than one cluster created this step"
        assert np.unique(parent_label).shape[0] == 1, "Parent cluster not unique"

        parent_label = parent_label[0]
        new_label = new_label[0]

        same_indices = np.where(labels == parent_label)

        # Add new node to parttree
        parent_depth = label_depths[parent_label]
        parttree.add_edge((i, new_label), mesh=changed_indices, parent_label=(parent_depth, parent_label))
        parttree.add_edge((i, parent_label), mesh=same_indices, parent_label=(parent_depth, parent_label))
        label_depths[new_label] = i
        label_depths[parent_label] = i
...
def solve_clustering(input_fname, uid, view_id, save_dir="test_results1", out_render_fol= "test_render_clustering", use_agglo=False, max_num_clusters=18, is_pc=False, option=1, with_knn=True, export_mesh=True, mesh_type="glb"):
    ...
    if not is_pc:
        ...
        mesh: trimesh.Trimesh = load_mesh_util(input_fname)
    ...
    if not use_agglo:
        print("Not implemented error. Agglomerative clustering only for hierarchical tree visualization.")
        exit()
        ...
        # Create PartTree
        parttree = hierarchical_clustering_to_tree(mesh, all_FL)

        out_tree_fol = os.path.join(out_render_fol, "tree", str(uid) + "_" + str(view_id))
        ...
        parttree.save_face_indices(os.path.join(out_tree_fol, "faces"))
        save_tree(parttree, fname_parttree)
